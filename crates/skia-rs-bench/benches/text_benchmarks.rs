...
fn bench_typeface(c: &mut Criterion) {
    ...
    group.bench_function("is_italic", |b| b.iter(|| black_box(&typeface).is_italic()));
    ...
    for text in [
        ...
        group.bench_with_input(
            ...
            &text,
            |b, &text| b.iter(|| black_box(&typeface).chars_to_glyphs(black_box(text))),
        );
    }
...
fn bench_font(c: &mut Criterion) {
    ...
    group.bench_function("size", |b| b.iter(|| black_box(&font).size()));

    group.bench_function("metrics", |b| b.iter(|| black_box(&font).metrics()));

    group.bench_function("spacing", |b| b.iter(|| black_box(&font).spacing()));
    ...
    for text in [
        ...
        group.bench_with_input(
            ...
            &text,
            |b, &text| b.iter(|| black_box(&font).measure_text(black_box(text))),
        );
    }
...
fn bench_font_style(c: &mut Criterion) {
    ...
    group.bench_function("normal", |b| b.iter(|| FontStyle::NORMAL));

    group.bench_function("bold", |b| b.iter(|| FontStyle::BOLD));

    group.bench_function("italic", |b| b.iter(|| FontStyle::ITALIC));

    group.bench_function("bold_italic", |b| b.iter(|| FontStyle::BOLD_ITALIC));

    let style = FontStyle::NORMAL;
    group.bench_function("weight", |b| b.iter(|| black_box(style).weight));

    group.bench_function("width", |b| b.iter(|| black_box(style).width));

    group.bench_function("slant", |b| b.iter(|| black_box(style).slant));
...
fn bench_text_blob(c: &mut Criterion) {
    ...
    for text in [
        ...
        group.throughput(Throughput::Elements(text.len() as u64));
        group.bench_with_input(BenchmarkId::new("build", text.len()), &text, |b, &text| {
            b.iter(|| {
                let mut builder = TextBlobBuilder::new();
...
fn bench_shaper(c: &mut Criterion) {
    ...
    for (name, text) in [
        ...
        group.bench_with_input(
            BenchmarkId::new("detect_direction", name),
            &text,
            |b, &text| {
                b.iter(|| {
