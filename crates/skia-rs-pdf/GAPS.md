...
**Severity:** Critical
**Status:** RESOLVED (with documented scope). `parse_truetype_metrics` now uses `ttf-parser` to read accurate `ascender` / `descender` / `italic_angle` / `cap_height` / `x_height` / global bbox, and per-code widths are built by looking up `glyph_index(ch)` + `glyph_hor_advance(gid)` scaled to PDF glyph units (1/1000 em). A deterministic FNV-1a-derived six-letter subset tag is prepended to the BaseFont name (e.g. `ABCDEF+Demo`) so the PDF marks the font as a subset. The writer compresses the full TTF data into a FontFile2 stream (with `/Length1` set to the uncompressed length) and wires it to the FontDescriptor. **Byte-level glyf/loca/cmap pruning is deferred** — the crate tracks `used_glyphs` per font (populated automatically from every `record_char` call) and is pre-wired for a future subsetter crate, but for now the embedded font is the full file. This is documented on the `PdfFont` type. Tested by `test_truetype_invalid_data_defaults`, `test_record_char_tracks_all_used`, and `truetype_embedding_emits_fontfile2_and_widths` (integration).
